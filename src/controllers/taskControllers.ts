...
export const taskControllers = {
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res
        .status(201)
        .json({ message: "task created sucessfully", taskCreated });
    } catch (error) {
      next(error);
    }
  },
  ...
  async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      //localhost:3000/task/jkjfjkfjkdsfk
      const { id } = UUIDSchema().parse(req.params);

      const userID = UUIDSchema().parse({ id: req.userID });

      if (status) {
        const taskUpdated = await taskServices.update(id,
          { title, description, date, status, user_id: userID.id},
          taskRepository
        );
        res
          .status(201)
          .json({ message: "task updated sucessfully", taskUpdated });
      }
    } catch (error) {
      next(error);
    }
  ...
  async delete(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const taskDeleted = await taskServices.delete(id,
         userID.id,
        taskRepository
      );
